...
From your Github account find the link to the repository, then clone your fork by using the ``git clone <URL>`` where the ``<URL>`` is the link you got from your repository
...
On the terminal do the following ``cd`` ``<space>`` ``<tab>`` ``lab7`` ``<tab>`` ``<enter>``. This opens open the folder which has what we just cloned. Then do ``bash`` ``<space>`` ``<tab>`` ``test`` ``<tab>`` ``<enter>``. This will run the test and will show failure.
